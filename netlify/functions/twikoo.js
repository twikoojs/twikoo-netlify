 ...
 */
exports.handler = async function (netlifyEvent, netlifyContext) {
  // event {
  ...
  try {
    ...
    await readConfig()
    headers = allowCors(netlifyEvent)
    if (netlifyEvent.httpMethod.toUpperCase() === 'OPTIONS') {
      return {
        statusCode: 204,
        headers,
        body: ''
...
function allowCors (request) {
  const corsHeaders = {}
  if (request.headers.origin) {
    corsHeaders['Access-Control-Allow-Credentials'] = true
    corsHeaders['Access-Control-Allow-Origin'] = getAllowedOrigin(request)
